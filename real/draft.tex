...
Our goal of predicting $L$ in some bin given the simulation output
...
frequency bins are narrow or equivalently that $p(L \cond \nu, \npack,
\bmnu, \bml)$ does not vary within a bin. This implies that every
sample $L_j$ in the bin follows the same distribution.  As the
...
Since $L$ is fully determined by $(N,\bmL)$, it does not depend on
$(\npack,\bml,\bmnu)$, i.e.
\begin{align}
  \lleq{pmi}
  p(L\cond N,\bmL,\nu,\npack,\bmnu, \bml)
...
\end{align}
and similarly for $L \to X$ because $\ell$ is bounded from above and
...
transform back to a probability density in $L$ via
\begin{align}
  \lleq{gmg}
