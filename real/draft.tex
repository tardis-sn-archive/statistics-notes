...
before. Writing the moments and variance of $L$ as\footnote{The $k$-th
  moment of variable $x$ is defined as the expectation value
  $\mu_k = \expect{x^k} = \int dx\,x^k\,p(x),\; k = 0,1,2,\ldots$.} %
\begin{align}
  \lleq{ram}
...
\end{align}
\hans{H: The last term in (\ref{rak}) was missing a square. Even so, I
  have not yet been able to verify that equation (rak) from
  Wikipedia's Normal Distribution entry agrees with Eq.(200) of
  Murphy's 2007 paper.} %
...
The above equations also show that the data $\bmy$ appears only in the
...
\end{align}
\hans{H: If threefold integrals must be carried out for the expectation values, why not do so
for Eq (ral) directly rather than just the expectation values?} %
