...
We consider \tardis{} as an example to demonstrate both how
...
small $\ell$. These features can be accomodated by a $\GammaDist$
...
\end{align}
where $ \epsilon=10^{-6}$. We infer $p(\Lumtot \cond n,\bml)$ and only
in the end transform back to a probability density in the raw
coordinates via
\begin{align}
  \lleq{gmg}
...
\end{align}
We also rescale the frequencies by a constant from
$\mathcal{O}(10^{15})$ to $\mathcal{O}(1)$ but the most important
...
distribution and thus $\Lumtot$ instead of $\Lumtot^\prime$ the
...
frequency. % $\nu_i$
This effect could be captured, for example, by a (linear) model such
$\alpha = \alpha(\nu \cond \bmphi)$ and
$\beta = \beta(\nu \cond \bmphi)$ are functions of $\nu$ that depend
on parameters $\bmphi$ such that integrals over $\alpha, \beta$ would
turn into integrals over $\bmphi$. But for the sake of clarity, let us
consider only the simplest case where $\alpha, \beta$ are the same in
every bin such that \refeq{spk} becomes
\begin{align}
  \lleq{gmi}
  p(Q\cond \npack,\bmy)
  &=     \sum_{N=0}^\infty \;
...
drastically reduced: instead of having $\alpha, \beta$ separately for
...
\end{align}
The case $N=0$ deserves special attention. To preserve the normalization of $p(Q\cond \npack,\bmy)$, we need to define
\begin{align}
  \lleq{gmk}
 ...
 \fred{Notation is sloppy before, it has nothing to do with unbinned
   ...
   me.}
 %
 Intuitively, we can only observe $Q=0$ for $N=0$ packets bu
...
\refeq{spj}, and apply Laplace's method to integrate over $N$ also.
 ...
 \subsection{Poisson-gamma convolution}\label{sec:poisson-gamma}

\hans{H: This subsection is temporary. It is motivated by the question
  whether the use of a gaussian for the single-packet luminosity
  $p(L_j\cond\bmphi)$ in (\ref{rae}) is necessary and whether making
  it a gaussian is a source of bias since $L_j$ cannot be
  negative. The original choice of a Gamma distribution as in
  (\ref{spc}) is more appropriate and could eliminate one source of
  systematic bias. Below, I have derived a closed form for
  $p(Q\cond \lambda,\bmphi)$ using the gamma-distributed
  $p(L_j\cond \alpha,\beta)$ of (\ref{spc}). The final result
  (\ref{qfp}) below could be used to replace the gaussian result
  (\ref{rab}) for insertion into (\ref{asd}). Inclusion of the entire
  derivation below is not envisaged; rather, we would include only
  salient issues and equation in the text itself. For the moment, the
  question is what Fred has to say about this version ie whether it
  can be implemented numerically and how it compares to the above
  results.

  The limit of this section to Gaussian can be investigated later if necessary.
}
