...
\fred{I removed a lot of cruft and swapped the symbols $x \leftrightarrow \ell$ so it's in line with the previous section}

Here we focus on the real packets as opposed to the virtual packets
and denote the raw output by \tardis{} for the $i$-{th} packet as $\qty(\nu_i^\prime, \ell_i^\prime)$.  A histogram plot \fredmargin{include plot} of the luminosities
from a \tardis{} run indicates a strongly skewed distribution with its
single mode close to the maximum luminosity $\lmax$, while there is a
long tail for small $\ell$. These features can be accomodated by a
$\GammaDist$ distribution if we transform the luminosities to
\begin{align}
  \lleq{gmf}
