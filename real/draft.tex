...
With these choices, the posterior is found to be
\begin{align}
  \lleq{spi}
  p(\alpha,\beta\cond n,\bml)
...
%%%%%%%
\fred{Inconsistency. Now $b_0$ is a scale parameter but before $\beta$
  was a \emph{rate} parameter in definition of $\Gamma$} 
%%%%%%%
\hans{Not really. This is an inverse gamma distribution, not a gamma
  distribution.  Transformation from $\sigma$ to gamma variable
  $v = 1/\sigma_2$ would make $b_0$ a rate variable for the latter.}
\begin{align}
  \lleq{rad}
...
\hans{H: This subsection is temporary. It is motivated by the question
  ...
  (\ref{ffin}) below could be used to replace the gaussian result
