...
\fred{Check below if \refeq{spk} used with flat prior}

\subsection{Preprocessing \tardis{} output } \label{sec:tardis}
...
A histogram plot of the luminosities from a \tardis{} run indicates a
strongly skewed distribution with its single mode close to the maximum
...
\fred{No, what I used before was equivalent to a uniform prior that
  ...
  the normalization but not the \emph{shape} of $p(Q)$} \fred{The
  error is fixed and I remove the next sections as Hans suggested.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\hans{H: I would agree to remove this section; I do not see value added.}
