...
variance $2 (n - a + 1)$. For our purposes, the variance is $ \approx
2 n$. The intuitive reason for this value is that the Poisson variance
appears twice, once when observing $n$ and once when predicting $N$
given $n$.  The region around $N=n$ yields the dominant contribution
to the sum over $N$ in \refeq{gmd}. The contribution near $N=0$ is
then negligible, and we can approximate $\sum_N$ by $\int
\rmdx{N}$. The density $p(N \cond n) P(X \cond N, \npack, \bmx,
...
\subsubsection*{Large $\bmn, \bmN$,  and large $\bmm$} \label{large-n-large-m}
