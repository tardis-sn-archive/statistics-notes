...
\fred{I removed $m$ whereever I saw it} We observe good agreement
between the best fit of a Gamma distribution to TARDIS packet
luminosities; an example is shown shown in \reffig{gamma}.  While not
obvious at first, a major numerical simplification arises because of
the $L \to X$ transformation. Rewriting the master
...
\begin{align}
  \lleq{gmc}
 p(X \cond N, \bmphi, \nu) &= \int d\bmX\,\delta(X - \textstyle\sum_j X_j)
...
\begin{align}
  ...
  \sum_N p(N\cond n)\, \int d\bmphi\, p(X \cond N, \bmphi, \nu)\, p(\bmx\cond \bmphi, \npack, \bmnu)
  \, p(\bmphi).
\end{align}
...
posterior is nearly insensitive to the prior choice for 30 samples in
the iid case. Translated to our problem, this would be 30 packets at
the same frequency. From TARDIS, we only get packets at different
frequencies. In the following, let $m$ denote the effective number of
packets close to $\nu$ where ``close'' is deliberately left
unspecified.  If $m$ is large, we can well determine $\phi$
and thus $p(X_j \cond N, \bmphi, \nu)$.
...
\begin{mnumerate}
  \item \label{large-m} For large $m$, the posterior $p(\bmphi \cond
  \npack, \bmx, \bmnu) = p(\bmx\cond \bmphi, \npack, \bmnu) \, p(\bmphi) / p(\bmx \cond \npack, \bmnu)$
  approaches a multivariate Gaussian. We can then compute the various
  moments $E_\bmphi[f(x)]$ with $f(x)=1,x,x^2$ by Laplace's method to
  good accuracy.
  \item \label{very-large-m} In the limit $m \to \infty$, the
  covariance of the posterior $p(\bmphi \cond \npack, \bmx, \bmnu)$
  tends to zero as $1/m$ and we can then fix the parameters $\bmphi$
  at the posterior mode $\bmphi^*$; i.e., $p(\bmphi \cond \npack,
  \bmx, \bmnu) \to \delta(\bmphi - \bmphi^*)$.
  ...
  we can replace $\sum_n \to \int \rmdx{N}$. The density $p(N \cond n) P(X
