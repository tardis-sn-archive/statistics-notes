...
\item In the simplest case, the frequency of channel $b$ is given by
  ...
  \fred{If in the end we want to compare to actual data from a
    telescope, I'd prefer to not call the TARDIS output ``data''. Why
    don't we just call it ``TARDIS output''?}  $D$, i.e.\ fixed
  results which we do not question but accept as given.  These
  ``data'' are the number of packets leaving the supernova, $\npack$,
  the luminosity $\bml$, and the frequencies $\bmnu$ for each packet
  \begin{align}
    \lleq{dfc}
    D = \{ \npack, \bml, \bmnu \} \,,
  \end{align}
  ...
  \begin{align}
    ...
    \dim \bml = \dim \bmnu = \npack \,.
  \end{align}
  Given $D$, the number of samples in bin $b$ is
  \begin{align}
    \lleq{dff}
    n_b = \sum_{j=1}^{\npack} \mathbf{1}_{[\nu_b,\nu_{b+1}]}( \nu_j )
  \end{align}
  and similarly the luminosity is
  \begin{align}
    \lleq{dfe}
    \ell_b = \sum_{j=1}^{\npack}\ell_j \mathbf{1}_{[\nu_b,\nu_{b+1}]}( \nu_j ),
  \end{align}
  where $\mathbf{1}_V(x)$ is the indicator function requiring $x \in V$.
\end{mtemize}
...
$p(N_b)$. \fred{In our general case, $\npack = \sum_b n_b$ is an instance of a
  ...
  not matter because we consider bins independently. } Similarly,
$\bml_b$ is a specific realization of a collection of stochastic
variables $\bmL_b =\{L_{b,1},\ldots,L_{b,\smN_b}\}$, where the number
$N_b$ of such variables is itself stochastic. The relevant pdf for
$\bmL_b$ therefore always presupposes knowledge of $N_b$, so we must
work with the conditional probability $p(\bmL_b\cond N_b)$. This
situation is captured mathematically as $N_b \sim p(N_b), \bmL_b \sim
p(\bmL_b\cond N_b)$ where $\sim$ means ``is by hypothesis distributed
in this model according to the pdf'', i.e.\
\begin{align*}
...
\begin{mtemize}
\item We omit the dependence on supernova parameters $\bmtheta$ from
  the notation below and instead condition on the TARDIS output
...
\item We further assume that individual packets are generated
  ...
  separately in each bin, we would have $B+1$ fits and $B+1$
  parameters. In each such fit, there would be only few samples to
  constrain $a$ in each bin. Then instead of fitting $a$ separately in
  each bin, we assume $a = a(\nu, \bmphi)$. This greatly reduces the
  number of parameters if $B$ is large because a handful of
  parameters, $\bmphi$, can be inferred from the complete set of
  packets. Necessarily, $a$ is best determined for the frequency
  around which the density of packets is highest.
...
calculations below. For non-integer $a$, the Negative Binomial
generalizes immediately to the Pólya distribution by replacing $x! \to
\Gamma(x+1)$. Note that the distribution of $N$ given $n$ is
independent of $\bmL$ and $\bml$ by construction.
...
Our goal of predicting $L$ given the TARDIS output $(\npack,\bml,
\bmnu)$ is captured in the probability $p(L\cond \npack,\bml, \bmnu)$
with all relevant parameters integrated out. We first note from
\refeq{pmh} that $p(N\cond \npack,\bml, \bmnu) = p(N\cond n)$ does not
depend on $\bml$ or $\bmnu$ directly but only through $n$, the number
of packets in the bin; cf. \refeq{dff}. Furthermore, since $L$ is
fully determined by $(N,\bmL)$,
\begin{align}
  \lleq{pmjd}
  L &= \sum_{j=1}^N L_j
\end{align}
is does not depend directly on $(\npack),\bml,\bmnu)$, i.e.\
\begin{align}
  \lleq{pmi}
  p(L\cond N,\bmL,n,\bml, \bmnu)
  = p(L\cond N,\bmL)
...
\begin{align}
  \lleq{pmj}
  p(L\cond n,\bml, \bmnu)
  &= \sum_{N=0}^\infty p(L\cond N,n,\bml, \bmnu)\,p(N\cond n,\bml, \bmnu) \\
  \lleq{pmjb}
  &= \sum_N p(N\cond n)\,\int d\bmL\, p(L\cond N,\bmL,n,\bml, \bmnu)\, p(\bmL\cond N,n,\bml, \bmnu)
  \\
  \lleq{pmjc}
  &= \sum_N p(N\cond n)\,\int d\bmL\,\delta(L - \textstyle\sum_{j=1}^N L_j)
  \, p(\bmL\cond N,n,\bml, \bmnu)
\end{align}
\fred{Add missing $\bmnu$! Everything that is conditional on $\bml$ is
  also conditional on $\bmnu$} The dependence on the TARDIS ``data''
$(\npack,\bml, \bmnu)$ enters through $p(\bmL\cond N,\npack,\bml,
\bmnu)$ in terms of the parameters $\bmphi$ of $p(\bmL\cond
N,\bmphi)$,
\begin{align}
  \lleq{pmk}
  p(\bmL\cond N,\npack,\bml, \bmnu)
  &= \int d\bmphi\, p(\bmL\cond N,\bmphi,\npack,\bml, \bmnu)\,p(\bmphi\cond N,\npack,\bml, \bmnu) \\
  &= \int d\bmphi\, p(\bmL\cond N,\bmphi)\,p(\bmphi\cond N,\npack,\bml, \bmnu).
\end{align}
...
\begin{align}
  \lleq{pmkb}
  p(\bmphi\cond N,\npack,\bml, \bmnu) &=
  \frac{p(\bml\cond N,\npack,\bmphi,\bmnu)\,p(\bmphi\cond N,\npack,\bmnu)} %
  {\int d\bmphi\,p(\bml\cond N,\npack,\bmphi,\bmnu)\,p(\bmphi\cond N,\npack,\bmnu)}, %
  % \ =\ \frac{p(\bml\cond N,\npack,\bmphi)\,p(\bmphi\cond N,n)} %
  % {p(\bml\cond n)} %
\end{align}

 a multiplicity-independent prior for $\bmphi$
\begin{align}
  \lleq{pml}
  p(\bmphi\cond N,\npack,\bmnu) &= p(\bmphi),
\end{align}
and recognizing that $N$ does not tell us anything about $\bml$
\begin{align}
  \lleq{pmla}
  p(\bml\cond N,\npack,\bmphi,\bmnu) = p(\bml\cond \npack,\bmphi,\bmnu)
\end{align}
...
\npack,\bmphi)\,p(\bmphi)$ in the denominator.
...
\begin{equation}
  ...
  \displaystyle\int d\bmphi\,p(\bmL\cond N,\bmphi) \, p(\bml\cond \npack,\bmphi)
  \, p(\bmphi)
  }
\end{equation}
with
\begin{align}
  p(\bmL\cond N,\bmphi) &= \prod_{j=1}^N p(L_j\cond \bmphi) \\
  p(\bml\cond \npack,\bmphi) &= \prod_{j=1}^n p(\ell_j\cond \bmphi)
\end{align}
...
take the largest luminosity from the entire frequency range and
increase it slightly to avoid numerical difficulties at the end point
\begin{align}
  \lleq{gme}
...
between adjacent $p(X \cond N, \bmx, m)$, so $p(X \cond n, \bmx, m)$
can be well approximated by a unimodal distribution; see
\reffig{hedgehog}.  \fred{I'm not 100 \% sure but $p(X \cond n, \bmx,
  m)$ should still have $N \to \infty$ modes but they aren't visible}
\missingfigure{from hedgehog to single peak \label{fig:hedgehog}}
...
form. This conjugate prior is thus of no use for us, and we turn to a
noninformative prior. \fred{Be explicit about prior choice} The
authors of \cite{moala2013bayesian} present an overview of possible
choices and conclude that the point estimate given by the mode of the
posterior is nearly insensitive to the prior choice for 30 samples. We
require in addition that the posterior is narrow enough so we group
samples in bins to have at least $m=150$ samples in each bin. This
implicitly assumes that the luminosity distribution is constant in the
frequency range defined by $m$.
...
parameter space spanned by $n$ and $m$.

\begin{mnumerate}
  \item \label{large-m} For $m$ large, the posterior $p(\bmphi \cond m, \bmx) =
  p(\bmx\cond m,\bmphi) \, p(\bmphi) / p(\bmx \cond m)$ approaches a
  multivariate Gaussian. We can then compute the various moments
  $E_\bmphi[f(x)]$ with $f(x)=1,x,x^2$ by Laplace's method to good
  accuracy.
  \item \label{very-large-m} In the limit $m \to \infty$, the covariance of the posterior
  $p(\bmphi \cond m, \bmx)$ tends to zero as $1/m$ and we can then fix
  the parameters $\bmphi$ at the posterior mode $\bmphi^*$; i.e.,
  $p(\bmphi \cond m, \bmx) \to \delta(\bmphi - \bmphi^*)$.
  \item \label{large-n} For $n$ large, the cumulative of the Negative
  Binomial $p(N \cond n)$ can be approximated by a Gaussian, defining
  a region around $N=n$ with the dominant contribution to the sum over
  $N$ in \refeq{gmd}. The contribution around $N=0$ is negligible, and
  we can replace $\sum_n \to \int dN$. The density $p(N \cond n) P(X
  \cond N,m, \bmx)$ is close to a Gaussian and the integral can be
  accurately approximated by Laplace's method which requires the
  Hessian and the mode. We benefit from partial error cancelation
  because we apply Laplace's method both in the numerator and
  denominator~\cite{tierney_accurate_1986} as each moment has to be
  normalized by the evidence $p(\bmx \cond m)$. The former is
  available in closed form and the latter has to be determined
  numerically. Fortunately, Newton's method leads to a solution in
  only a few steps~\cite{minka2002estimating}. \fred{The reference is
    for max. likelihood of a $\GammaDist(\cdot)$ but I'm sure the
    performance is similar for max. posterior and the first moments}
  \item \label{central-limit} The central limit theorem states that for large $N$,
...
% Local Variables:
% compile-command:"rubber --pdf -W refs -W misc -S bayes-tardis-luminosity-prediction"
% End:
