...
\item In the simplest case, the frequency of channel $b$ is given by
  ...
  \item We treat the results of a single TARDIS run as \textit{data}
    ...
    \hans{I agree, ``data'' is awkward, and it is indeed useful to
      ...
      $S$.}
    %%%%%
    $D$, i.e.\ fixed results which we do not question but accept as
    given.  This ``data'' consists of the total number of packets
  ...
  \begin{align}
    ...
    \dim \bml = \dim \bmnu = \npack \,.
  \end{align}
  In terms of the $\mathbf{1}_V(x)$ the indicator function requiring
...
\end{mtemize}
\hans{Discussion of rebinning with index $m$?}
...
\item We further assume that individual packets are generated
  ...
  \hans{I would replace the word ``sample'' above and throughout the
    text by ``packet''. The word ``sample'' refers in statistics to
    the entire data set $D$, not just to one data point.} %
...
\end{align}
\hans{So far, $m$ appears only in your todo comment above. I would suggest
  a short discussion of this already in Section \ref{sec:dfnr}. We
  could then simply rewrite all equations below in terms of $m$ rather
  than $b$.} %
