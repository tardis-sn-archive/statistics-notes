...
\item In general, the distribution of $L_j$ could be a of a different
  functional form in each bin. But in practice, the distribution of
  packets does not change abruptly with frequency. We therefore assume
  that the functional form of $p(L_j)$ is identical in each bin but it
  is parametrized by a continuous function of frequency $f(\nu |
  \bmphi)$. This greatly reduces the number of parameters if $\npack$
  is large because a handful of parameters, $\dim \bmphi$, can be
  inferred from the complete set of packets instead
