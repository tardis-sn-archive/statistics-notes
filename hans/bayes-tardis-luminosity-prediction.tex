...
\item We treat the results of a single TARDIS run as \textit{data}
  ...
  given.  These ``data'' are the number of packets leaving the
  supernova, $\npack$, the luminosity $\bml$, and the frequencies
  $\bmnu$ for each packet
  \begin{align}
    \lleq{dfc}
    D = \{ \npack, \bml, \bmnu \} \,,
  \end{align}
...
\begin{align}
  ...
  % {p(\bml\cond n)} %
\end{align}
\fred{$p(\bmphi\cond N,n,\bml) = p(\bmphi\cond n,\bml)$. Below $N$ is (correctly) missing in \refeq{pmp} }
