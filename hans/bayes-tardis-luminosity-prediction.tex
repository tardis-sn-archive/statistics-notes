...
\end{align}
For our purposes, the most important property is that the Gamma distribution is infinitely divisible; i.e., the sum of $N$ Gamma variates is again a Gamma variate
\begin{align}
  \lleq{gmb}
  X_i \sim Gamma(\alpha, \beta) \Rightarrow \sum_{i=1}^N X_i \sim Gamma(N \alpha, \beta)
\end{align}

With $\bmphi = (\alpha, \beta)$, the property \refeq{gmb} is
equivalent to
\begin{align}
  \lleq{gmc}
 p(L \cond N, \bmphi) &= \int d\bmL\,\delta(L - \textstyle\sum_j L_j)
  \,p(\bmL\cond N,\bmphi) = Gamma(L \cond N \alpha, \beta)
\end{align}
...
\begin{align}
  \lleq{gmd}
    p(L\cond n,\bml,m)
  = \frac{1}{p(\bml\cond n)}
  \sum_N p(N\cond n)\, \int d\bmphi\, p(L \cond \bmphi)\, p(\bml\cond m,\bmphi)
  \, p(\bmphi).
\end{align}
...
noninformative prior. In \cite{Moala:2013}, an overview of possible
choices is given. They conclude that the point estimate given by the
mode of the posterior is nearly insensitive to the prior choice for 30
samples. We require in addition that the posterior is narrow enough so
we group samples in bins to have at least $m=150$ samples in each
bin. The following approximations can be used in different regions of
the parameter space spanned by $n$ and $m$.
